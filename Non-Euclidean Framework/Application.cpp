...
#pragma region RTT_CUBES
    std::function<void( uint32_t i, uint32_t j )> RenderStencils = std::function( [&]( uint32_t i, uint32_t j ) -> void
    {
        // Render RTT cube
    ...
    uint32_t threadCount = m_bUseStencilCube ? CAMERA_COUNT : RENDER_DEPTH;
